...
def test_epsilon_sq_boot_ci() -> None:
    ...
    epsilon_sq_boot_ci must equal _epsilon_sq_kw exactly.
    """
    from strategies.vol_regime_switch.regime_population import (
        _epsilon_sq_kw,
        epsilon_sq_boot_ci,
    ...
    with pytest.warns(RuntimeWarning, match="excludes its point estimate"):
        point, lo, hi = epsilon_sq_boot_ci(
            pred_nl, regime_nl, block=10, n_boot=2000, seed=43
        )
    ...
    assert lo < hi, f"CI bounds must be ordered: lo={lo}, hi={hi}"
    assert point > hi, f"fixture must expose percentile bias: point={point}, hi={hi}"
