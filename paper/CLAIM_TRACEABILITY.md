...
| C9 | Recentering the decision statistic on the empirical null lowers the cMDE ~tenfold, restores size **at the declared window `W=120`**, and yields the protocol's first `admissible` certificate for a bounded `q=2` claim, scoped to the calendar-and-volume scheme pair; real BTC still abstains (powered exclusion). | exploratory | `recentered_reference_repair_20260710.json`, generated and audited by `scripts/wp1/reference_repair.py`. Constants are post-freeze — confirmatory rerun required. OOS size is `0.02` at `W=120` but `0.12` at `W=60` and `0.08` at `W=240`, so the window scoping is load-bearing (`prereg/DEVIATIONS.md` D13). Transport for this claim is `2/3` pairs, not `3/3` (D1). |
