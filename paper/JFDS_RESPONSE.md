...
Missing required gates now produce `incomplete` even when a measured gate has already failed. Observed failures remain in the failure profile. The rolling-quantile exhibit had no power, size, or transport measurement, so its disposition is incomplete. The negative value result is retained only as a failure-profile entry.
...
The repaired \(q=2\) record is `superseded_exploratory`. It is not defended as a first admissible certificate. A protocol-v5 successor with \(\alpha_0=0.025\), frozen recentering constants, calendar/volume scope, and repository-unseen 2026-06-01 through 2026-07-31 BTC data replaces it.

## Issue 6 — Complete EUR/USD rolling-quantile certificate

EUR/USD is the second asset-class/detector-family certificate. Protocol v5 freezes the detector, clocks, gates, and numerical rules before evaluation-span download. The certificate is generated complete regardless of admission or withholding.
...
| BTC \(q=2\) successor | repaired record | 0 | 0.025 | pending unseen evaluation |
| EUR/USD rolling quantile | — | 0 | 0.025 | pending unseen evaluation |
