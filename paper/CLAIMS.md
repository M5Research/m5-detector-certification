...
> Certificate-grade information inference uses a stationary-block reference. At
> evidentiary revision 0, the BTC successor returns `E=0.801` at `q=2` and `E=0.620`
> at `q=5`, both below the revision threshold `1/alpha_0 = 40`; EUR/USD returns
> `E=111.803` and clears that threshold.
...
> The protocol-v5 BTC successor does not support detector-contingent information at
> `q=2` or `q=5`: both stationary-block confidence intervals include zero and both
> e-values fall below 40. The older IID-permutation artifact is retained only as
> `legacy_noncertificate` provenance.

Allowed:

> The complete EUR/USD rolling-quantile certificate supports its information and
> net-value gates, but is withheld as `size_distorted` under the fixed refusal
> precedence; passing downstream gates cannot override the size failure.

Historical, legacy-only wording (not allowed for the protocol-v5 certificate):

> The frozen VR trigger has small detector-contingent MI at `q=2` and `q=5`;
...
> EUR/USD certification requires bid/ask data (HistData Generic ASCII ticks); daily FRED
> or ECB reference rates are not cost-gate evidence. Protocol v5 includes a complete
> second-asset rolling-quantile certificate on an unseen 2026 evaluation span. It is
> withheld as `size_distorted`, not described as admissible.
...
> negative), converting an underpowered withhold into a powered exclusion. That repaired
> `q=2` record is `superseded_exploratory`. Protocol-v5 admission, if any, comes only from
> the confirmatory successor with `alpha_0=0.025` and previously unseen 2026-06/07 data.

Allowed:

> The confirmatory BTC successor is complete and withheld as `target_mismatched`.
> Its target failure has precedence over its later transport, information, and value
> failures; the full failure profile remains visible in the certificate.

Not allowed:

> The repaired `q=2` result is the protocol's first admissible certificate.
...
> No identified financial framework jointly binds power, size, transport, target
> information, and decision value to one frozen detector-output claim with refusal
> precedence. Gauge R&R and ISO 5725 are prior measurement-system practice; they are not
> that joint financial-detector certificate.
